...
async function PackContainers(request) {
	return $.ajax({
		url: '/api/containerpacking/multibox',
		type: 'POST',
		data: request,
		...
		PackContainers(JSON.stringify(request))
			.then(boxes => {
				...
				const results = boxes.map(box => {
					return {
						Container: containers.find(container => container.ID() === box.ContainerID),
						PackingResult: box.AlgorithmPackingResults[0]
                    }
				});
				self.PackedResults(results);
			});
	};
